...
def ApplyesKFold(x_axis, y_axis, hsv, binary):
    ...
    # print("\nRandom Forest: {0}\nMulti-layer Perceptron: {1}\nknn Boosting: {2}".format(
    #     np.round(np.mean(randomForest_result), 2),  np.round(np.mean(mplc_result), 2),  np.round(np.mean(knn_result), 2)))

    # print("\nThe best model is: {0} with value: {1}".format(
